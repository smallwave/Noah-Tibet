module module_ascii_io
  ...
  character(len=4096) :: varstring    !wuxb update 2018->4096    2015.10.3
  character(len=4096) :: unitstring   !wuxb update 2018->4096    2015.10.3
  character(len=4096) :: fmtstring    !wuxb update 2018->4096    2015.10.3
  integer             :: lenstring
...
!------------------------------------------------------------------------------------------------
  ...
  subroutine open_forcing_file(iunit, output_dir, filename, infotext, nlayers, startdate, enddate,           &
    ...
    integer, pointer,    dimension(:) :: stype   !wuxb  add  2015.9.30
    ...
    integer, target, dimension(100) :: soil_htype        = -99999   !wuxb  add 2015.9.30
    integer :: i
    integer :: nlayers_temperature
    integer :: nlayers_moisture
    integer :: nlayers_liquid
    integer :: nlayers_stype     !wuxb  add 2015.9.30
    logical :: lexist

    ! add  soil_htype  2015.9.30
    ...
    if ( ( nlayers_temperature /= nlayers ) .or. &
         ...
         ( nlayers_stype       /= nlayers ) ) then   !wuxb add 2015.9.30
    ...
    nullify (smc)
    nullify (sh2o)
    nullify (stype)     ! wuxb add 2015.9.30
    nullify (sldpth)
    ...
    stype  => soil_htype(1:nlayers)    ! wuxb add 2015.9.30
