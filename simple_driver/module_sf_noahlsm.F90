...
! 3 -> j75 old
  INTEGER :: OPT_TCND = 3  !(suggested ?)
    ...
    !  STYPE(NSOIL) SOIL TEXTURE CLASSFICATION FOR EVERY HORIZON
    !
    ...
    !   STYPENUM   SOIL TYPE NUMBER, EXTRACT SOIL PARAMATER
    ! ----------------------------------------------------------------------
    ...
    INTEGER, DIMENSION(1:NSOIL), INTENT(IN) :: stype    ! wuxb add 2015.9.30
    ...
    REAL :: Z0MIN,     Z0MAX
    INTEGER :: stpnum ! wuxb add 2015.10.2
    ! ----------------------------------------------------------------------
    ...
    !wuxb add 2015.10.2
    !soil parameter:PSISAT=SATPSI; BEXP=BB; DKSAT=SATDK; DWSAT=SATDW;
    !SMCMAX=MAXSMC; SMCWLT=WLTSMC; SMCREF=REFSMC; SMCDRY=DRYSMC; F1=F11; QUARTZ=QTZ
    ...
    !wuxb 2015.10.2
    !
    !CALL REDSTP (stpnum,  PSISAT, BEXP,DKSAT,DWSAT,          &
        !             SMCMAX,SMCWLT,SMCREF,SMCDRY,F1,QUARTZ)
        !write(*,'("SMCMAX=''", F11.7, "''")') SMCMAX
        stpnum = stype(1)
        IF(OPT_TCND == 1)  THEN
            CALL TDFCND_C05 (DF1,SMC (1),QUARTZ,SMCMAX,SH2O (1),stpnum)
        END IF
        IF(OPT_TCND == 2) THEN
            CALL TDFCND_ORI (DF1,SMC (1),QUARTZ,SMCMAX,SH2O (1))
        END IF
		IF(OPT_TCND == 3) THEN
            CALL TDFCND (DF1,SMC (1),QUARTZ,SMCMAX,SH2O (1),stpnum)
        END IF

        !urban
        IF ( VEGTYP == ISURBAN ) DF1=3.24

        ! 2015.12.16 WUXIAOBO ADD 
        !IF(SHDFAC > 0) THEN
        !    SBETA = - 0.5 * XLAI/SHDFAC
        !END IF
        !IF(SHDFAC == 0) THEN
        !    SBETA = 0
        !END IF
        ...
        IF (SNEQV  == 0.0) THEN
            !wuxb add 2015.10.2
            CALL NOPAC (stype,ETP,ETA,PRCP,SMC,SMCMAX,SMCWLT,            &
        ...
        ELSE
            !wuxb add 2015.10.2
            CALL SNOPAC (stype,ETP,ETA,PRCP,PRCPF,SNOWNG,SMC,SMCMAX,SMCWLT,    &
                SMCREF,SMCDRY,CMC,CMCMAX,NSOIL,DT,              &
                SBETA,DF1,                                      &
    ...
    ! ----------------------------------------------------------------------
    !wuxb add stype  2015.10.2
    SUBROUTINE HRT (stype,RHSTS,STC,SMC,SMCMAX,NSOIL,ZSOIL,YY,ZZ1,          &
    ...
    INTEGER              :: I, K
    INTEGER              :: stpnum !wuxb add 2015.10.2
    REAL                 :: DKSAT,DWSAT,SMCDRY, SMCREF,SMCWLT !wuxb add 2015.10.2
    REAL                 :: BEXP, F1,PSISAT,QUARTZ, SMCMAX
    REAL, INTENT(IN)     :: CSOIL, DF1, DT, TBOT,YY,ZZ1, ZBOT
    ! wuxb delete 2015.10.2
    ! REAL, INTENT(IN)     :: BEXP, CSOIL, DF1, DT,F1,PSISAT,QUARTZ,     &
        !                         SMCMAX ,TBOT,YY,ZZ1, ZBOT

        REAL, DIMENSION(1:NSOIL), INTENT(IN)   :: SMC,STC,ZSOIL
        INTEGER, DIMENSION(1:NSOIL), INTENT(IN)   :: stype ! wuxb add 2015.10.2
        ...
        DO K = 2,NSOIL
            ...
            IF (K /= NSOIL) THEN
                ...
                ! ----------------------------------------------------------------------
                IF(OPT_TCND == 1)  THEN
                     CALL TDFCND_C05 (DF1N,SMC (K),QUARTZ,SMCMAX,SH2O (K),stpnum)
                END IF
                IF(OPT_TCND == 2) THEN
                     CALL TDFCND_ORI (DF1N,SMC (K),QUARTZ,SMCMAX,SH2O (K))
                END IF
		        IF(OPT_TCND == 3) THEN
                    CALL TDFCND (DF1N,SMC (K),QUARTZ,SMCMAX,SH2O (K),stpnum)
                END IF
                !urban
    ...
    ! ----------------------------------------------------------------------
    !wuxb add 2015.10.2
    SUBROUTINE NOPAC (stype,ETP,ETA,PRCP,SMC,SMCMAX,SMCWLT,           &
    ...
    IF (ETP > 0.0) THEN
        ...
        !wuxb add stype 2015.10.2
        CALL SMFLX (stype,SMC,NSOIL,CMC,DT,PRCP1,ZSOIL,                &
    ...
    !urban
    !wuxb add stype 2015.10.2
    CALL SHFLX (stype, SSOIL,STC,SMC,SMCMAX,NSOIL,T1,DT,YY,ZZ1,ZSOIL,   &
    ...
    DO I = 1,NROOT
        ...
        !RTDIS (I) = (1-0.9** (- SLDPTH (I)))/(1-0.9** (ZSOIL (NROOT)))
        ! ----------------------------------------------------------------------
	...
	! wuxb add 2015.10.2
	! extract soil parameters according to soil type
	SUBROUTINE REDSTP (stpnum,PSISAT,BEXP,DKSAT,DWSAT,                   &
		SMCMAX,SMCWLT,SMCREF,SMCDRY,F1,QUARTZ)

	IMPLICIT NONE

	! Soil parameters
	INTEGER, INTENT(IN)    :: stpnum
	REAL, INTENT(OUT)      :: BEXP,DKSAT,DWSAT,F1,QUARTZ,SMCDRY,          &
		SMCMAX,SMCREF,SMCWLT,PSISAT
	...
	DWSAT = SATDW (stpnum)
	F1 = F11 (stpnum)
	PSISAT = SATPSI (stpnum)
	QUARTZ = QTZ (stpnum)
	SMCDRY = DRYSMC (stpnum)
	SMCMAX = MAXSMC (stpnum)
	SMCREF = REFSMC (stpnum)
	SMCWLT = WLTSMC (stpnum)

	END  SUBROUTINE REDSTP
	...
	!wuxb add 2015.10.2
	SUBROUTINE SHFLX (stype,SSOIL,STC,SMC,SMCMAX,NSOIL,T1,DT,YY,ZZ1,ZSOIL, &
		TBOT,ZBOT,SMCWLT,PSISAT,SH2O,BEXP,F1,DF1,     &
		QUARTZ,CSOIL,VEGTYP,ISURBAN)
	...
	!REAL, INTENT(IN)      :: BEXP,CSOIL,DF1,DT,F1,PSISAT,QUARTZ,     &
		...
		! Land case
		! wuxb   add  stype  2015.10.2
		CALL HRT (stype,RHSTS,STC,SMC,SMCMAX,NSOIL,ZSOIL,YY,ZZ1,TBOT,        &
			ZBOT,PSISAT,SH2O,DT,                                &
	...
	! ----------------------------------------------------------------------
	!wuxb add stype  2015.10.2
	SUBROUTINE SMFLX (stype,SMC,NSOIL,CMC,DT,PRCP1,ZSOIL,             &
		&                   SH2O,SLOPE,KDT,FRZFACT,                        &
		&                   SMCMAX,BEXP,SMCWLT,DKSAT,DWSAT,                &
		&                   SHDFAC,CMCMAX,                                 &
		&                   RUNOFF1,RUNOFF2,RUNOFF3,                       &
    ...
    !wuxb update   2015.10.2
    REAL, INTENT(IN)      :: CMCMAX, DT, EC, EDIR, KDT, PRCP1, SHDFAC, SLOPE
	REAL                  :: BEXP, DKSAT,DWSAT, SMCMAX, SMCWLT


	REAL, INTENT(OUT)     :: DRIP, RUNOFF1, RUNOFF2, RUNOFF3
	REAL, INTENT(INOUT)   :: CMC
	REAL, DIMENSION(1:NSOIL), INTENT(IN)   :: ET,ZSOIL
	INTEGER, DIMENSION(1:NSOIL), INTENT(IN)   :: stype  !wuxb add stype 2015.10.2
	...
	REAL :: FAC2
	REAL :: FLIMIT
	REAL :: F1,QUARTZ,SMCDRY,SMCREF,PSISAT
	...
	IF ( ( (PCPDRP * DT) > (0.0001*1000.0* (- ZSOIL (1))* SMCMAX) )   &
		.OR. (FAC2 > FLIMIT) ) THEN
	     !wuxb add stype 2015.10.2
	     CALL SRT (stype,RHSTT,EDIR,ET,SH2O,SH2O,NSOIL,PCPDRP,ZSOIL,          &
		     DWSAT,DKSAT,SMCMAX,BEXP,RUNOFF1,                    &
		     RUNOFF2,DT,SMCWLT,SLOPE,KDT,FRZFACT,SICE,AI,BI,CI)
	     !wuxb add stype 2015.10.2
	     CALL SSTEP (stype,SH2OFG,SH2O,DUMMY,RHSTT,RHSCT,DT,NSOIL,SMCMAX,     &
		     CMCMAX,RUNOFF3,ZSOIL,SMC,SICE,AI,BI,CI)
	     DO K = 1,NSOIL
		     SH2OA (K) = (SH2O (K) + SH2OFG (K)) * 0.5
	     END DO
	     !wuxb add stype 2015.10.2
	     CALL SRT (stype,RHSTT,EDIR,ET,SH2O,SH2OA,NSOIL,PCPDRP,ZSOIL,         &
		     DWSAT,DKSAT,SMCMAX,BEXP,RUNOFF1,                    &
		     RUNOFF2,DT,SMCWLT,SLOPE,KDT,FRZFACT,SICE,AI,BI,CI)
         !wuxb add stype 2015.10.2
         CALL SSTEP (stype,SH2O,SH2O,CMC,RHSTT,RHSCT,DT,NSOIL,SMCMAX,         &
		     CMCMAX,RUNOFF3,ZSOIL,SMC,SICE,AI,BI,CI)
    ...
    IF (OPT_FRZ == 1) THEN
        IF(TAVG < T0) THEN
            CALL FRH2O_NY06(FREE,TAVG,SMCMAX,BEXP,PSISAT,SMC)
        ELSE
	...
	! ----------------------------------------------------------------------
	!wuxb add stype 2015.10.2
	SUBROUTINE SNOPAC (stype,ETP,ETA,PRCP,PRCPF,SNOWNG,SMC,SMCMAX,SMCWLT,   &
		SMCREF,SMCDRY,CMC,CMCMAX,NSOIL,DT,                  &
		SBETA,DF1,                                          &
		Q2,T1,SFCTMP,T24,TH2,FDOWN,F1,SSOIL,STC,EPSCA,      &
	...
	T11 = T1
	!wuxb add 2015.10.2
	CALL SHFLX (stype,SSOIL1,STC,SMC,SMCMAX,NSOIL,T11,DT,YY,ZZ1,ZSOIL,     &
		TBOT,ZBOT,SMCWLT,PSISAT,SH2O,BEXP,F1,DF1,           &
		QUARTZ,CSOIL,VEGTYP,ISURBAN)
	...
	! ----------------------------------------------------------------------
	!wuxb add 2015.10.2
    SUBROUTINE SRT (stype,RHSTT,EDIR,ET,SH2O,SH2OA,NSOIL,PCPDRP,            &
	...
	INTEGER, DIMENSION(1:NSOIL)  :: stype !wuxb 2015.10.2
    ...
    REAL        :: SMCWXB,SMCWXB2,SMCWXB3   !WUXB ADD 2015.11.17
	...
	IF (PCPDRP /=  0.0) THEN
        ...
        !
        ! FCR FROM SCHAAKE AND KOREN MODEL. 
        ! EQ 24  WUXB ADD
		! ----------------------------------------------------------------------
        ...
        IF(OPT_INF == 1)  THEN
            SMCWXB =  SH2OA(1)  + SICE(1)
            CALL WDFCND_NY06 (WDF,WCND,SMCWXB,SMCMAX,BEXP,DKSAT,DWSAT,SICE(1))
        END IF
		IF(OPT_INF == 2) THEN
	...
	DO K = 2,NSOIL
		...
		IF (K /= NSOIL) THEN
            ...
            !write(*,'("SMCMAX=''", F11.7, "''")') SMCMAX
            IF(OPT_INF == 1) THEN
                SMCWXB2 =  SH2OA(K)  + SICE(K)
                CALL WDFCND_NY06 (WDF2,WCND2,SMCWXB2,SMCMAX,BEXP,DKSAT,DWSAT,SICE(K))
            END IF
			IF(OPT_INF == 2) THEN
		...
		ELSE
			...
			!write(*,'("SMCMAX=''", F11.7, "''")') SMCMAX
            IF(OPT_INF == 1) THEN 
                SMCWXB3 =  SH2OA(NSOIL)  + SICE(NSOIL)
                CALL WDFCND_NY06 (WDF2,WCND2,SMCWXB3,SMCMAX,BEXP,DKSAT,DWSAT,SICE(K))
            END IF
			IF(OPT_INF == 2) THEN 
	...
	! ----------------------------------------------------------------------
	!wuxb 2015.10.2
	SUBROUTINE SSTEP (stype,SH2OOUT,SH2OIN,CMC,RHSTT,RHSCT,DT,              &
	...
	REAL, INTENT(IN)          :: CMCMAX, DT
	REAL                      :: SMCMAX ! wuxb 2015.10.2
	...
	REAL     :: BEXP,DKSAT,DWSAT,F1,QUARTZ,SMCDRY,SMCREF,SMCWLT,PSISAT  ! wuxb add 2015.10.2
	...
	REAL, INTENT(IN) :: TUP, TM, TDN,ZSOIL(NSOIL)
	REAL     X0,  XDN, XUP ,DZ, DZH  ,T0
	...
	SGX = 0.0
	DO I = 1,NROOT
		GX (I) = ( SMC (I) - SMCWLT ) / ( SMCREF - SMCWLT )  !Fsw
	...
	DENOM = 0.
	DO I = 1,NROOT
		RTX = RTDIS (I) + GX (I) - SGX    ! RTDIS -> Froot,i        RTX -> Frootm,i
		GX (I) = GX (I) * MAX ( RTX, 0. )
		DENOM = DENOM + GX (I)
	END DO
	...
	! ----------------------------------------------------------------------
#define _SIMPLE_DRIVER_ 
#ifdef _SIMPLE_DRIVER_
