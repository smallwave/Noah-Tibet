...
#   wuxb
# REVISION HISTORY
#    20160923 -- Initial version created and posted online
#
# REFERENCES
##########################################################################################################
library(sp)
...
permafrostMapDemTemp <- merge(permafrostMapDem,subDemExtractDf,by.x="ID",by.y="ID")
...
for (i in 1:lenDemCls) 
{
  ...
  resYOA[1]<- end
  dfSel <- subset(permafrostMapDemTemp, dem_tibet > first & dem_tibet < end & X1983Map == 1,select = c(ID, X1983Map,Type,PermafrostChangeTemp))
  lendfSel <-nrow(dfSel)
  resYOA[2] <-lendfSel
  #oa
...
#************************************************************************************************
## Write res
outPutName <- paste("F:/worktemp/Permafrost(Change)/Work/Res/CVS/TempDem.csv",sep='')
